import random
lower_letters_ru = 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя'
upper_letters_ru = 'АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'
...
def check_letters():
   while True:
      ...
      if n in lower_letters_ru or n in upper_letters_ru:
         return n.upper()
      else:
         print('Русские буквы ептэ')

def check_word():
   while True:
      s = input('Введите слово полностью:\n')
      if s.isalpha():
         for i in s:
            if i.upper() not in upper_letters_ru:
               print('Только русские слова')
               break
         else:
            return s.upper()
      else:
         print('Только буквы, без всей хуйни')

def choice():
   while True:
      n = input('Хотите называть букву или слово полностью? (б/с)\n').lower()
      if n == 'б' or n == 'с':
         return n
      else:
         print('Варианты ответов в скобках, б - буква/ с - слово :)')


def replacement_array(word, letter):
   return [i for i in range(len(word)) if word[i] == letter]

def repeat():
   while True:
      n = input('Сыграем еще раз? (д/н)\n')
      if n.upper() == 'Д':
         return play()
      elif n.upper() == 'Н':
         return print('Пока')
      else:
         print('Варианты смотри...')
...
def play():
   ...
   guessed_words = []
   tries = 6
   print('Давай сыграем в игру "Угадайка"\nНужно угадать слово загаданное программой.')
   print(display_hangman(tries))
   while '_' in word_completion and tries > 0:
      print(''.join(word_completion))
      print('Буквы, которые были использованны:', guessed_letters)
      print('Слова, которые вы называли:', guessed_words)
      guess = choice()
      if guess =='б':
         letter = check_letters()
         if letter in guessed_letters:
            print('Уже была такая буква, лох :Р')
         else:
            if letter in word:
               guessed_letters.append(letter)
               replacement = replacement_array(word, letter)
         ...
         all_word = check_word()
         if all_word in guessed_words:
            print('Уже называл это слово жопич :3')
         else:
            if all_word == word:
               return print('Поздравляю вы угадали слово!! Словом было', word)
            else:
               guessed_words.append(all_word)
               print('Не, дружок, думай еще...')
               tries -= 1
               print(display_hangman(tries))
   print(word)
   return repeat()
